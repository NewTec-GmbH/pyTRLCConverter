...
- [2 Example Use Cases](#2-example-use-cases)
  - [2.1 Conversion of TRLC files to Markdown format UC1](#21-conversion-of-trlc-files-to-markdown-format-uc1)
  - [2.2 Conversion of TRLC files to docx format using a stored user defined document template UC2](#22-conversion-of-trlc-files-to-docx-format-using-a-stored-user-defined-document-template-uc2)
  - [2.3 Conversion of TRLC files to reStructuredText format UC3](#23-conversion-of-trlc-files-to-restructuredtext-format-uc3)
...
- UC1) Conversion of TRLC files to Markdown format.
- UC2) Conversion of TRLC files to docx format using a stored user defined document template.
- UC3) Conversion of TRLC  files to reStructuredText format.
...
Following potential violations may occur during executing the tool ***pyTRLCConverter***:
- a) After conversion process the requirement or parts of it are lost.
- b) After conversion process the requirement or parts of it are corrupted. 
- c) After conversion process the reference to another requirement is lost or corrupted. 
- d) With the possibility to convert requirements to a format with using project specific conversion files (.json and .py files), after conversion process the requirement content is lost or corrupted.
- e) Using a corrupted document template to export to the format .docx,  after conversion process the requirement content is lost or corrupted. 
...
For all these potential violations a, b and c it is not possible for the user of the tool to detect these errors upon reviewing of the converted requirement because the review process of writing requirements is already finished. 
...
For all these potential violations d and e it would be necessary to  detect errors to review the project specific conversion .JSON and .py files and to review the project specific document templates (docx template).
The user has only low possibility to detect a tool error so the Tool Error Detection is very low **TD3**.
...
### 2.1 Conversion of TRLC files to Markdown format UC1
|||
|---|---|
...
|**Environmental, Functional and Process Constraints**|***pyTRLCConverter*** GPL-3.0.<br>***Python version***: The required python version is defined in ***pyproject.toml***<br> ***Operating system: cross-platform***. Rational: The tool runs equally well on Windows, Linux and macOS, due to the fact that a python interpreter is installed. <br>|
...
### 2.2 Conversion of TRLC files to docx format using a stored user defined document template UC2
|||
|---|---|
...
### 2.3 Conversion of TRLC files to reStructuredText format UC3
|||
|---|---|
